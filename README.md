 ...
 * To reproduce the same results, you must run the program by navigating to the directory of the docs folder on the repo and run the command
   * python -m streamlit run app.py
 * However, for the program to work, you must make sure you have ollama installed and running the model Llama 3.2  on your device (Llama 3.3 must be installed in the environment initialization). Also, you must have the streamlit, ollama and scikit-learn python libraries installed for the program to work. If the program does not work on your device, check to make sure you have ollama installed and running with the model Llama 3.2 as well as the necessary external libraries for the program installed.
 * After launching the phishing Detector, click on any email on the left, then click "scan with Llama 3.2" to run the LLM powered phishing detector. The algorithmic phishing detector already runs automatically upon startup, so you can simply look at the number under "algorithm" to see the algorithms phishing detection analysis.
